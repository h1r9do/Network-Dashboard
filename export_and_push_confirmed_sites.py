...
def export_confirmed_sites():
    """Export all confirmed sites from database"""
    print("=== Exporting confirmed sites from database ===")
    ...
    try:
        # Query confirmed sites with MX devices
        query = session.query(
            ...
            (EnrichedCircuit.wan1_confirmed == True) | (EnrichedCircuit.wan2_confirmed == True),
        ...
        print(f"✅ Exported {len(sites_to_push)} confirmed sites")
...
def main():
    """Main function"""
    print("=== Separate Push Operation for Confirmed Sites ===")
