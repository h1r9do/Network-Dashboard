#!/usr/bin/env python3
"""
MODIFIED Nightly Enriched Database Script with DSR-ARIN Preservation
This version preserves DSR data when ARIN providers match
Instead of truncating, it does smart updates
"""
...
def enrich_circuits(conn):
    """MODIFIED: Main enrichment process with preservation logic"""
    cursor = conn.cursor()
    ...
    for device in devices:
        ...
        # For WAN2: If current provider is DSR and ARIN matches, preserve it
        wan2_preserve = False
        if (current_record.get('wan2_confirmed') and 
            current_record.get('wan2_provider') and 
            wan2_arin and
            providers_match_for_sync(current_record.get('wan2_provider'), wan2_arin)):
            wan2_preserve = True
...
def main():
    """Main function"""
    logger.info("=== Starting MODIFIED Nightly Enriched Database Script ===")
    logger.info("Using preservation logic for DSR-ARIN matched data")
