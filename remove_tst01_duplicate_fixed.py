#!/usr/bin/env python3
"""
Remove duplicate default rule from TST 01 - Fixed version
The issue was case sensitivity: "any" vs "Any" in protocol field
Both rules 55 and 56 are functionally identical default rules
"""
...
def remove_duplicate_default_rule():
    ...
    for i, (idx1, idx2) in enumerate(duplicates):
        print(f"\nDuplicate pair {i+1}:")
        print(f"  Rule {idx1+1}: {rules[idx1]}")
        print(f"  Rule {idx2+1}: {rules[idx2]}")
...
def main():
    """Main function"""
    print("TST 01 Duplicate Rule Removal - Fixed Version")
    print("=" * 50)
    ...
    for i, (idx1, idx2) in enumerate(duplicates):
        print(f"\nDuplicate {i+1}:")
        print(f"  Rule {idx1+1}: {rules[idx1].get('comment', 'No comment')} - {rules[idx1].get('protocol', 'N/A')}")
        print(f"  Rule {idx2+1}: {rules[idx2].get('comment', 'No comment')} - {rules[idx2].get('protocol', 'N/A')}")
    
    print(f"\nThis will remove {len(duplicates)} duplicate rule(s)")
    print(f"Rules count: {len(rules)} -> {len(rules) - len(duplicates)}")
    
    confirm = input("\nType 'YES' to proceed with duplicate removal: ")
    if confirm == 'YES':
