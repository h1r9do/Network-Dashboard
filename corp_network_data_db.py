...
def get_comprehensive_datacenter_inventory_db():
    """Get comprehensive datacenter inventory from inventory_web_format table"""
    # Import the new function
    import sys
    sys.path.insert(0, '/usr/local/bin')
    from get_inventory_web_format_db import get_inventory_web_format_db
    
    # Use the new function to get data from inventory_web_format table
    return get_inventory_web_format_db()

def get_comprehensive_datacenter_inventory_db_old():
    """OLD VERSION - Get comprehensive datacenter inventory with SSH inventory integrated (master/module hierarchy)"""
    conn = None
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
        ...
        for row in cursor.fetchall():
            ...
            # Fix N2K devices - they should be blade components, not masters
            if model and "N2K" in model:
                # Skip N2K devices here - they will be added as children from SSH data
                continue
        ...
        try:
            ...
            for ip, device_info in ssh_data.items():
                ...
                for blade in chassis_blades:
                    # Skip blades with empty models
                    if not blade.get("model") or blade.get("model") in ["", """", "Unknown", "N/A"]:
                        continue
                    # Skip if model is empty or invalid
                    model_value = blade.get('model', '').strip()
                    if not model_value or model_value in ['""', "''", 'Unknown', 'N/A', 'null', 'NULL']:
                        continue
                    
                    # Debug: Log what we're seeing in blade models
                    model_debug = blade.get('model', 'NOT_SET')
                    if not model_debug or model_debug in ['', '""', "''", 'Unknown', 'N/A', 'null', 'NULL']:
                        print(f"🐛 DEBUG: Skipping blade with model='{model_debug}', repr={repr(model_debug)}", file=sys.stderr)
                        continue
                    else:
                        print(f"🐛 DEBUG: Keeping blade with model='{model_debug}'", file=sys.stderr)
                    ...
                    inventory.append({
                        ...
                        'model': blade.get('model', '') if blade.get('model', '') != '""' else 'Unknown',
                        'software_version': '',
        ...
        # Add SNMP collected inventory data
        print("📡 Loading SNMP inventory from database...")
        snmp_devices_added = 0
        snmp_components_added = 0
        ...
        try:
            ...
            # Check which devices are already in inventory (from SSH or datacenter_inventory)
            existing_hostnames = set(item['hostname'] for item in inventory if item['hostname'])
            
            for device in snmp_devices:
                hostname = device[0]
                ip_address = str(device[1])
                collection_timestamp = device[2]
                model = device[3] or ''
                software_version = device[4] or ''
                system_description = device[5] or ''
                collection_method = device[6] or 'SNMP'
                physical_components = device[7] or {}
                component_counts = device[8] or {}
                status = device[9]
                ...
                vendor = 'Cisco' if 'Cisco' in system_description else 'Unknown'
                ...
                inventory.append({
                    ...
                    'announcement_date': None,
                    'component_counts': {
                        'chassis_blades': component_counts.get('modules', 0),
                        'sfp_modules': component_counts.get('transceivers', 0),
                        'hardware_components': component_counts.get('power_supplies', 0) + component_counts.get('fans', 0),
                        'interfaces': 0
                    },
                    'parent_device': '',
                    'module_number': '',
                    'ports': '',
                    'interface_name': '',
                    'description': f'SNMP Collection ({collection_method})'
                })
                snmp_devices_added += 1
                
                # Process physical components
                if physical_components:
                    # Extract chassis serial number if available
                    if 'chassis' in physical_components:
                        for chassis in physical_components['chassis']:
                            if chassis.get('serial_number'):
                                # Update the master device serial number
                                inventory[-1]['serial_number'] = chassis['serial_number']
                                break
                    ...
                    if 'modules' in physical_components:
                        for module in physical_components['modules']:
                            ...
                            # Skip modules with empty models
                            if not module.get("model_name") or module.get("model_name") in ["", '""', "Unknown", "N/A"]:
                                continue
                                # Skip if model is empty or invalid
                                model_value = module.get('model_name', '').strip()
                                if not model_value or model_value in ['""', "''", 'Unknown', 'N/A', 'null', 'NULL']:
                                    continue
                        ...
                        for fex in physical_components['fex_units']:
                            ...
                            if not fex.get("model") or fex.get("model") in ["", '""', "Unknown", "N/A"]:
                                ...
                                inventory.append({
                                    ...
                                    'component_counts': {},
                                    'parent_device': hostname,
                                    'module_number': fex.get('name', ''),
                                    'ports': '',
                                    'interface_name': fex.get('name', ''),
                    ...
                    if 'transceivers' in physical_components:
                        for transceiver in physical_components['transceivers']:
                            if transceiver.get('serial_number'):
                                inventory.append({
                                    'device_type': 'sfp',
                                    'site': site,
                                    'hostname': f"  └─ Transceiver",
                                    'vendor': '',
                                    'mgmt_ip': '',
                                    ...
                                    'module_number': '',
                                    'ports': '',
                                    'interface_name': transceiver.get('name', ''),
                                    'description': transceiver.get('description', '')
                                })
                                snmp_components_added += 1
                    
                    # Add power supplies
                    if 'power_supplies' in physical_components:
                        for ps in physical_components['power_supplies']:
                            if ps.get('serial_number'):
                                inventory.append({
                                    'device_type': 'component',
                                    'site': site,
                                    'hostname': f"  └─ Power Supply",
                                    'vendor': '',
                                    'mgmt_ip': '',
                                    ...
                                    'component_counts': {},
                                    'parent_device': hostname,
                                    'module_number': ps.get('name', ''),
                                    'ports': '',
                                    'interface_name': ps.get('name', ''),
                                    'description': ps.get('description', '')
                                })
                                snmp_components_added += 1
                
        except Exception as e:
            print(f"⚠️ Could not load SNMP inventory: {e}")
            import traceback
            traceback.print_exc()
        
        print(f"📡 SNMP devices added: {snmp_devices_added}, components added: {snmp_components_added}")
        ...
        return {
            ...
            'summary': {
                ...
                'ssh_devices': devices_with_components,
                'snmp_devices': snmp_devices_added,
                'snmp_components': snmp_components_added
            }
        }
...
def get_collected_inventory_db():
    ...
    try:
        ...
        cursor.execute("""
            SELECT hostname, hostname as ip, name, description, pid, vid, serial_number
        ...
        # Get modules
        cursor.execute("""
            SELECT hostname, hostname as ip, module_number, module_type, model, status, serial_number
        ...
        # Get SFPs
        cursor.execute("""
            SELECT hostname, hostname as ip, interface, sfp_type, vendor, serial_number, part_number
            FROM collected_sfps
            WHERE collection_id = %s
        ...
        # Get FEX modules
        cursor.execute("""
            SELECT parent_hostname, parent_hostname as ip, fex_number, model, serial_number, description, state
        ...
        # Get power supplies
        cursor.execute("""
            SELECT hostname, hostname as ip, name, description, pid, status, serial_number
