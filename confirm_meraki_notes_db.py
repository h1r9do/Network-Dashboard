...
def confirm_site(site_name):
    ...
    try:
        ...
        # Get tracking data from circuits table
        result = session.execute(text("""
        ...
        # Get costs from tracking data by matching providers
        wan1_cost = '$0.00'
        wan2_cost = '$0.00'
        
        # Match WAN1 provider to tracking data
        ...
        # Match WAN2 provider to tracking data
        wan2_provider = enriched_data[6] or ''
        if wan2_provider:
            for track in tracking_data:
                if track['provider_name'] and wan2_provider.upper() in track['provider_name'].upper():
                    if track['billing_monthly_cost']:
                        try:
        ...
        # Build CSV data for modal display
        csv_data = []
        for track in tracking_data:
            csv_data.append({
                'Site Name': site_name,
                'Provider Name': track['provider_name'],
                'Speed': track['speed'],
                'Price': f"${float(track['billing_monthly_cost']):.2f}" if track['billing_monthly_cost'] else '$0.00',
                'DSR Circuit Purpose': track['Circuit Purpose'],
                'Status': 'Enabled',
        ...
        response_data = {
            ...
            'wan1_provider_dsr': tracking_data[0]['provider_name'] if tracking_data else '',
            'wan1_speed_notes': enriched_data[3] or '',
            'wan1_speed_dsr': tracking_data[0]['speed'] if tracking_data else '',
            ...
            'wan2_provider_dsr': tracking_data[1]['provider_name'] if len(tracking_data) > 1 else '',
            'wan2_speed_notes': enriched_data[7] or '',
            'wan2_speed_dsr': tracking_data[1]['speed'] if len(tracking_data) > 1 else '',
