...
def check_non_live_sites():
    ...
    cursor.execute("""
        ...
        WHERE (ec.wan1_ip IS NULL OR ec.wan1_ip = '' OR ec.wan1_ip = 'N/A')
        AND (ec.wan2_ip IS NULL OR ec.wan2_ip = '' OR ec.wan2_ip = 'N/A')
        ORDER BY ec.network_name
        LIMIT 20
    """)
    
    no_ip_sites = cursor.fetchall()
    print(f"\nFound sites without any IP addresses (showing first 20):")
    for site in no_ip_sites:
        print(f"  {site[0]}: Device Status={site[3]}, WAN1={site[4]}, WAN2={site[5]}")
    
    # Count total sites without IPs
    cursor.execute("""
        SELECT COUNT(*)
        FROM enriched_circuits
    ...
    print(f"\nTotal sites without any IP addresses: {total}")
