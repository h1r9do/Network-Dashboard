#!/usr/bin/env python3
"""
TEST VERSION: Merged Meraki collection + enrichment script
Limited to 10 Discount Tire stores for testing
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/test-merged-enriched.log'),
...
# TEST LIMIT - Process only 10 stores
TEST_LIMIT = 10
...
def enrich_device(device_data, dsr_circuits_by_site, conn):
    ...
    if wan1_flipped and wan2_flipped:
        logger.info(f"{network_name}: WAN flip detected during enrichment")
...
def main():
    ...
    logger.info("=== Starting TEST Merged Meraki + Enrichment Script ===")
    logger.info(f"Limited to {TEST_LIMIT} Discount Tire stores for testing")
    ...
    try:
        ...
        # We'll collect networks that have Discount-Tire tagged devices
        # First, let's get all devices to check their tags
        logger.info("Searching for Discount Tire tagged networks...")
        
        dt_networks_set = set()  # Use set to avoid duplicates
        dt_networks = []
        ...
        for net in networks:
            ...
            # Skip hubs, labs, test stores
            if any(pattern in net_lower for pattern in ['hub', 'lab', 'voice', 'datacenter', 'test', 'store in a box', 'sib']):
                continue
            
            # Get devices for this network
            devices = get_devices(net_id)
            
            # Check if any device has Discount-Tire tag
            for device in devices:
                if device.get('model', '').startswith('MX'):
                    # Get device details to check tags
                    device_details = get_device_details(device.get('serial'))
                    if device_details:
                        tags = device_details.get('tags', [])
                        # Check for Discount-Tire tag only
                        if 'Discount-Tire' in tags:
                            if net_id not in dt_networks_set:
                                dt_networks_set.add(net_id)
                                dt_networks.append(net)
                                logger.info(f"Found Discount Tire store: {net_name} (tags: {tags})")
                                if len(dt_networks) >= TEST_LIMIT:
                                    break
            
            if len(dt_networks) >= TEST_LIMIT:
                break
        
        dt_networks.sort(key=lambda net: (net.get('name') or "").strip())
        logger.info(f"Found {len(dt_networks)} Discount Tire networks for testing")
        
        devices_processed = 0
        
        for net in dt_networks:
            net_name = (net.get('name') or "").strip()
            net_id = net.get('id')
            ...
            for device in devices:
                ...
                if model.startswith("MX"):
                    ...
                    if store_device_in_db(device_entry, conn):
                        devices_processed += 1
                        logger.info(f"  Stored device {serial} - WAN1: {wan1_ip} ({wan1_provider}), WAN2: {wan2_ip} ({wan2_provider})")
                        
                        # IMMEDIATELY ENRICH THIS DEVICE
                        enrich_device(device_entry, dsr_circuits_by_site, conn)
                        logger.info(f"  Enriched device {serial}")
                        
                        # Commit every device for testing
                        conn.commit()
                        logger.info(f"  Committed to database (device {devices_processed})")
                        sys.stdout.flush()
        ...
        logger.info(f"\n=== TEST COMPLETE ===")
        logger.info(f"Processed {devices_processed} devices from {len(dt_networks)} networks")
        logger.info(f"Data stored in both meraki_inventory and enriched_circuits tables")
