#!/usr/bin/env python3
"""
BETA VERSION: Meraki collection + enrichment in one script
- Uses BETA tables for safe testing  
- Exact copy of production merged script but with beta table names
- Same logic, same functionality, safe for testing
"""
...
def get_organization_uplink_statuses(org_id):
    ...
    while True:
        ...
        logger.info(f"Page {len(all_statuses)//1000 + 1}: Fetched {len(statuses)} devices, total so far: {len(all_statuses)}")
        ...
        if statuses and 'serial' in statuses[-1]:
            ...
            logger.info(f"Next page cursor (serial): {statuses[-1]['serial']}")
        else:
            break
...
def store_device_in_db(device_data, conn):
    """Store device data in BETA tables"""
    cursor = conn.cursor()
    
    try:
        # Store in meraki_inventory_beta table with IP, ARIN data, and parsed notes
        insert_sql = """
        ...
        # Also store ARIN data in RDAP cache beta
        for wan in ['wan1', 'wan2']:
            ...
            if ip and provider and provider != "Unknown":
                cursor.execute("""
                    INSERT INTO rdap_cache_beta (ip_address, provider_name, last_queried)
                    VALUES (%s, %s, NOW())
                """, (ip, provider))
...
def collect_vlan_firewall_data(network_id, network_name, conn):
    """Collect VLANs and firewall rules for a specific network - integrated version"""
    vlans = []
    rules = []
    ...
    try:
        ...
        vlans_response = make_api_request(url, MERAKI_API_KEY)
        if vlans_response and isinstance(vlans_response, list):
            vlans = vlans_response
    except Exception as e:
        if "400" not in str(e):  # Only log non-400 errors (400 = no MX device)
            logger.error(f"Failed to fetch VLANs for {network_name}: {e}")
    
    try:
        # Get firewall rules  
        url = f"{BASE_URL}/networks/{network_id}/appliance/firewall/l3FirewallRules"
        rules_response = make_api_request(url, MERAKI_API_KEY)
        if rules_response and isinstance(rules_response, dict):
            rules = rules_response.get('rules', [])
    except Exception as e:
        if "400" not in str(e):  # Only log non-400 errors 
            logger.error(f"Failed to fetch firewall rules for {network_name}: {e}")
...
def enrich_device(device_data, dsr_circuits_by_site, conn):
    """Enrich a single device with DSR data and store in enriched_circuits_beta table"""
    cursor = conn.cursor()
    ...
    # Update or insert enriched record in BETA table
    if current_record:
        # Update existing record
        cursor.execute("""
            UPDATE enriched_circuits_beta SET
                wan1_provider = %s,
                wan1_speed = %s,
                wan1_circuit_role = %s,
                wan1_confirmed = %s,
...
def main():
    ...
    logger.info("=== Starting BETA Merged Meraki + Enrichment Script ===")
    ...
    try:
        ...
        for net in networks:
            ...
            # Check if network has MX devices for VLAN/firewall collection
            has_mx = False
                ...
                if model.startswith("MX"):
                    ...
                    if store_device_in_db(device_entry, conn):
                        ...
                        if devices_processed % 10 == 0:
                            conn.commit()
                            logger.info(f"Progress: Processed {devices_processed} devices, enriched {enriched_count}")
                            sys.stdout.flush()
            
            # Collect VLANs and firewall rules for this network if it has MX devices
            if has_mx:
                net_vlans, net_rules = collect_vlan_firewall_data(net_id, net_name, conn)
                vlan_count += net_vlans
                rule_count += net_rules
        
        # Commit all changes
        conn.commit()
        conn.close()
        
        logger.info(f"\n=== BETA PROCESSING COMPLETE ===")
        logger.info(f"Processed {devices_processed} devices and enriched {enriched_count} circuits")
        logger.info(f"Collected {vlan_count} VLANs and {rule_count} firewall rules")
        logger.info(f"Data stored in BETA tables: meraki_inventory_beta, enriched_circuits_beta, network_vlans_beta, firewall_rules_beta")
