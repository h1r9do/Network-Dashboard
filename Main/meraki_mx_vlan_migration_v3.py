#!/usr/bin/env python3
"""
Meraki MX VLAN Migration Script v3
==================================
...
New in v3:
- Applies firewall rules from template
- Handles VLAN number translation in firewall rules
- Supports separate firewall template file

Usage:
    python3 meraki_mx_vlan_migration_v3.py --network-id <network_id> --source-config <json_file> --firewall-template <json_file> --mode <test|production>

Example:
    python3 meraki_mx_vlan_migration_v3.py --network-id L_12345 --source-config azp_30_config.json --firewall-template firewall_rules_template.json --mode test
...
class VLANMigrator:
    def __init__(self, network_id, mode='production'):
        ...
        self.log(f"VLAN Migrator v3 initialized in {self.mode.upper()} mode")
    ...
    def migrate_vlans(self, source_vlans, source_policies=None):
        ...
        """
        self.log("Starting VLAN migration v3...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Meraki MX VLAN Migration Tool v3')
    ...
    try:
        ...
        # Apply firewall rules if template provided
        if firewall_template:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        print(__doc__)
        print("\nExample usage:")
        print("  python3 meraki_mx_vlan_migration_v3.py --network-id L_12345 --source-config azp_30_config.json --firewall-template firewall_rules_template.json --mode test")
        print("  python3 meraki_mx_vlan_migration_v3.py --network-id L_12345 --source-config azp_30_config.json --mode production")
    else:
        main()