...
def create_vlans(network_id, vlans_config):
    ...
    if default_vlan:
        ...
        # Clean up fields that can't be set
        for field in ['networkId', 'mask']:
    ...
    for vlan in vlans_config:
        ...
        # Update DHCP relay IPs if present
        if vlan_data.get('dhcpRelayServerIps'):
            vlan_data['dhcpRelayServerIps'] = [
                update_appliance_ip_for_test(ip) for ip in vlan_data['dhcpRelayServerIps']
            ]
...
def update_switch_ports(network_id, devices, azp_switch_config):
    ...
    # Get first switch config from AZP 30 as template
    azp_ports_config = list(azp_switch_config['ports'].values())[0]['ports']
    ...
    for switch in tst_switches:
        ...
        for port_config in azp_ports_config:
            ...
            # Clean up fields
            for field in ['portId', 'linkNegotiationCapabilities']:
