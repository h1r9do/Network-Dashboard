...
def dsrcircuits():
    ...
    """
    try:
        # Get enriched circuits data with ARIN provider information
        enriched_circuits = db.session.query(EnrichedCircuit, MerakiInventory).join(
            MerakiInventory,
...
def dsrallcircuits():
    ...
    try:
        ...
        sites_with_multiple_circuits = len([count for count in site_circuit_counts.values() if count > 2])
        total_circuits_at_multi_sites = sum([count for count in site_circuit_counts.values() if count > 2])
...
def refresh_arin_data(site_name):
    ...
    try:
        ...
        if not meraki_device or not meraki_device.device_serial:
            return jsonify({'success': False, 'error': f'No Meraki device found for {site_name}'}), 404
        ...
        try:
            ...
            return jsonify({'success': False, 'error': f'Meraki API error: {str(e)}'}), 500
            ...
            if device_status.get('serial') == device_serial:
                ...
                for uplink in uplinks:
                    if uplink.get('interface') == 'wan1':
                        ...
                        wan1_ip = uplink.get('publicIp') or uplink.get('ip')
                    elif uplink.get('interface') == 'wan2':
                        # Prefer publicIp over ip (interface IP might be private)
                        wan2_ip = uplink.get('publicIp') or uplink.get('ip')
                break
        ...
        wan1_arin_provider = query_arin_rdap(wan1_ip) if wan1_ip else 'Unknown'
        wan2_arin_provider = query_arin_rdap(wan2_ip) if wan2_ip else 'Unknown'
        
        # Update meraki_inventory table
        meraki_device.wan1_ip = wan1_ip
        meraki_device.wan2_ip = wan2_ip  
        meraki_device.wan1_arin_provider = wan1_arin_provider
        meraki_device.wan2_arin_provider = wan2_arin_provider
        meraki_device.last_updated = datetime.utcnow()
        
        db.session.commit()
        
        logger.info(f"Refreshed ARIN data for {site_name}: WAN1={wan1_ip}({wan1_arin_provider}), WAN2={wan2_ip}({wan2_arin_provider})")
        
        return jsonify({
            'success': True,
            'message': f'ARIN data refreshed for {site_name}',
            'wan1_ip': wan1_ip or 'N/A',
            'wan1_arin_provider': wan1_arin_provider,
            'wan2_ip': wan2_ip or 'N/A', 
