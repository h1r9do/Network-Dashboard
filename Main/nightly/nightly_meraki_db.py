...
def make_api_request(url, api_key, params=None, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Return both data and headers for pagination support
...
def make_api_request_simple(url, api_key, params=None, max_retries=5):
    """Wrapper for backward compatibility - returns just the data"""
    data, headers = make_api_request(url, api_key, params, max_retries)
    return data
...
def get_organization_id():
    ...
    orgs = make_api_request_simple(url, MERAKI_API_KEY)
    for org in orgs:
...
def get_organization_uplink_statuses(org_id):
    ...
    while True:
        page_count += 1
        statuses = make_api_request_simple(url, MERAKI_API_KEY, params)
        if not statuses:
            break
        ...
        # Use serial from last device as cursor (proven to work!)
...
def get_all_networks(org_id):
    ...
    params = {'perPage': 1000, 'startingAfter': None}
    while True:
        networks = make_api_request_simple(url, MERAKI_API_KEY, params)
        if not networks:
            break
...
def get_devices(network_id):
    ...
    return make_api_request_simple(url, MERAKI_API_KEY)
...
def resolve_private_ip_via_ddns(network_id, wan_number):
    ...
    try:
        ...
        data = make_api_request_simple(url, MERAKI_API_KEY)
        if data:
    ...
    try:
        ...
        for network in mx_networks:
            ...
            try:
                ...
                vlans = make_api_request_simple(url, MERAKI_API_KEY)
                ...
                if vlans and isinstance(vlans, list):
                    ...
                    for vlan in vlans:
                        ...
                        vlan_id = vlan.get('id')
                        if not vlan_id or vlan_id == '' or not str(vlan_id).isdigit():
                            logger.warning(f"Skipping VLAN with invalid ID '{vlan_id}' in network {network_name}")
                            continue
                        ...
                        try:
                            ...
                            logger.error(f"Error inserting VLAN {vlan_id} for network {network_name}: {vlan_error}")
                            conn.rollback()  # Rollback just this transaction
                ...
                ports = make_api_request_simple(url, MERAKI_API_KEY)
                ...
                conn.commit()  # Commit after each network to prevent data loss
    ...
    try:
        ...
        for network in mx_networks:
            ...
            try:
                ...
                response = make_api_request_simple(url, MERAKI_API_KEY)
                ...
                if model.startswith("MX"):
                    ...
                    if store_device_in_db(device_entry, conn):
                        ...
                        if devices_processed % 10 == 0:
                            conn.commit()
                            logger.info(f"Progress: Processed {devices_processed} devices (committed to database)")
