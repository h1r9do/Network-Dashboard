#!/usr/bin/env python3
"""
PRODUCTION VERSION: Complete Merged Meraki + Enrichment Script
- Combines nightly_meraki_db.py and nightly_enriched_db.py functionality
- Includes circuit change detection for maximum efficiency
- Processes ALL networks/devices with API speed optimization  
- Writes to PRODUCTION tables (meraki_inventory + enriched_circuits)
- Replaces both separate nightly scripts
"""
...
# PRODUCTION MODE - Use production tables
BETA_MODE = False
logger.info("🏭 PRODUCTION MODE - Writing to production tables")

def get_table_name(base_name):
    """Get table name (production mode - no suffix)"""
    return base_name
...
def main():
    """Main execution function for production"""
    logger.info("🏭 Starting PRODUCTION Merged Meraki + Enriched Script")
    logger.info("📝 Writing to PRODUCTION TABLES")
    ...
    try:
        ...
        logger.info(f"🚀 Processing {len(networks)} networks for FULL PRODUCTION SCALE testing...")
