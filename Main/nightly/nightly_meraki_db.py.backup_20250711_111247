...
def make_api_request(url, api_key, params=None, max_retries=5):
    """Enhanced API request with adaptive rate limiting for maximum speed"""
    headers = get_headers(api_key)
    
    # Adaptive rate limiting variables
    if not hasattr(make_api_request, 'current_delay'):
        make_api_request.current_delay = 0.1  # Start at 100ms (10x faster)
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code == 429:  # Rate limited
                ...
                # Slow down significantly when rate limited
                old_delay = make_api_request.current_delay
                make_api_request.current_delay = min(5.0, make_api_request.current_delay * 2.0)
                logger.warning(f"Rate limited! Slowing down: {old_delay:.3f}s -> {make_api_request.current_delay:.3f}s")
                time.sleep(10)  # Wait 10 seconds when rate limited
                continue
            ...
            if make_api_request.successful_requests % 5 == 0:
                old_delay = make_api_request.current_delay
                make_api_request.current_delay = max(0.05, make_api_request.current_delay * 0.95)
                if old_delay != make_api_request.current_delay:
                    logger.debug(f"Speeding up: {old_delay:.3f}s -> {make_api_request.current_delay:.3f}s")
