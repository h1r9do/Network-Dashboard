...
def confirm_site(site_name):
    ...
    try:
        ...
        if meraki_data[3]:  # device_tags from meraki_inventory
            # PostgreSQL returns array as string like '{tag1,tag2}' or as list
            if isinstance(meraki_data[3], list):
                device_tags = meraki_data[3]
            elif isinstance(meraki_data[3], str):
                # Remove curly braces and split by comma
                tags_str = meraki_data[3].strip('{}')
                if tags_str:
                    device_tags = [tag.strip() for tag in tags_str.split(',')]
            else:
                device_tags = []
        ...
        response_data = {
            ...
            'device_serial': meraki_data[0] or '',  # device_serial for tag updates
