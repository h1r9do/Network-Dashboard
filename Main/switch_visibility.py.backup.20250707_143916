...
def get_meraki_organization_id():
    ...
    response = requests.get(url, headers=meraki_headers)
...
def refresh_switch_data(serial):
    ...
    logger.info(f"Refreshing data for switch {serial}")
    
    try:
        # Get device details
        url = f'{MERAKI_BASE_URL}/devices/{serial}'
        device_response = requests.get(url, headers=meraki_headers)
        
        if device_response.status_code != 200:
            return {'error': f'Failed to get device details: {device_response.text}'}, 400
        
        device = device_response.json()
        
        # Get clients connected to this switch
        url = f'{MERAKI_BASE_URL}/devices/{serial}/clients'
        clients_response = requests.get(url, headers=meraki_headers)
        
        if clients_response.status_code != 200:
            return {'error': f'Failed to get clients: {clients_response.text}'}, 400
        
        clients = clients_response.json()
        
        # Get network details
        url = f'{MERAKI_BASE_URL}/networks/{device["networkId"]}'
        network_response = requests.get(url, headers=meraki_headers)
        ...
        return {
            'success': True,
            'message': f'Successfully refreshed {updated_count} clients for switch {device.get("name", serial)}',
            'clients_updated': updated_count
        }, 200
        
    except Exception as e:
        logger.error(f"Error refreshing switch {serial}: {str(e)}")
        db.session.rollback()
...
def refresh_store_data(store_name):
    ...
    logger.info(f"Refreshing data for store {store_name}")
    
    try:
        # Get organization ID
        org_id = get_meraki_organization_id()
        if not org_id:
            return {'error': 'Organization not found'}, 404
        
        # Get all networks
        url = f'{MERAKI_BASE_URL}/organizations/{org_id}/networks'
        networks_response = requests.get(url, headers=meraki_headers)
        
        if networks_response.status_code != 200:
            return {'error': f'Failed to get networks: {networks_response.text}'}, 400
        
        networks = networks_response.json()
        
        # Find the network for this store
        target_network = None
        for network in networks:
            if network['name'] == store_name:
                target_network = network
                break
        
        if not target_network:
            return {'error': f'Store {store_name} not found'}, 404
        
        # Get all devices in the network
        url = f'{MERAKI_BASE_URL}/networks/{target_network["id"]}/devices'
        devices_response = requests.get(url, headers=meraki_headers)
        
        if devices_response.status_code != 200:
            return {'error': f'Failed to get devices: {devices_response.text}'}, 400
        ...
        for device in devices:
            ...
            total_switches += 1
            result, status_code = refresh_switch_data(device['serial'])
        ...
        return {
            'success': True,
            'message': f'Successfully refreshed {total_switches} switches with {total_clients} clients in {store_name}',
            'switches_updated': total_switches,
            'clients_updated': total_clients
        }, 200
        
    except Exception as e:
        logger.error(f"Error refreshing store {store_name}: {str(e)}")
