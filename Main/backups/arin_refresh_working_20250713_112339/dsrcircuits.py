...
from dsrcircuits_beta_combined import dsrcircuits_beta_bp
...
from subnets_blueprint import subnets_bp
from eol_routes import eol_bp
from switch_visibility import switch_visibility_bp
from dsrcircuits_test import dsrcircuits_test_bp

from dsrcircuits_dev import dsrcircuits_dev_bp
from inventory_working import inventory_working_bp

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Import VLAN migration blueprints
try:
    from vlan_migration_routes import vlan_migration_routes
    from vlan_migration_api import vlan_migration_bp
    vlan_migration_available = True
except ImportError:
    logger.warning("VLAN migration module not available")
    vlan_migration_available = False

# Import VLAN migration test blueprints
try:
    from vlan_migration_test_routes import vlan_migration_test_routes
    from vlan_migration_test_api import vlan_migration_test_bp
    vlan_test_available = True
except ImportError:
    logger.warning("VLAN migration test module not available")
    vlan_test_available = False
...
def create_app(config_name='production'):
    ...
    app.register_blueprint(dsrcircuits_beta_bp)
    ...
    app.register_blueprint(dsrcircuits_test_bp)
    app.register_blueprint(dsrcircuits_dev_bp)
    app.register_blueprint(inventory_working_bp)
    
    # Register VLAN migration blueprints if available
    if vlan_migration_available:
        app.register_blueprint(vlan_migration_routes)
        app.register_blueprint(vlan_migration_bp)
        logger.info("✓ VLAN Migration module registered")
    
    # Register VLAN test blueprints if available
    if vlan_test_available:
        app.register_blueprint(vlan_migration_test_routes)
        app.register_blueprint(vlan_migration_test_bp)
        logger.info("✓ VLAN Migration Test module registered")
    ...
    def documentation_content():
        ...
        try:
            import markdown
            with open('/usr/local/bin/Main/README.md', 'r') as f:
                content = f.read()
    ...
    def serve_documentation(filename):
        ...
        try:
            ...
            # First check in /usr/local/bin
            file_path = os.path.join('/usr/local/bin', filename)
            
            # If not found, check in /usr/local/bin/Main/pages
