...
def refresh_arin_data(site_name):
    ...
    try:
        ...
        # Get ARIN providers from meraki_inventory table (already processed by nightly script)
        # This ensures consistency with the enriched data
        if meraki_device:
            # Use the ARIN data from meraki_inventory which was populated by nightly_meraki_db.py
            # with sophisticated parse_arin_response() logic
            result['wan1_arin_provider'] = meraki_device.wan1_arin_provider or 'Unknown'
            result['wan2_arin_provider'] = meraki_device.wan2_arin_provider or 'Unknown'
            logger.info(f"Using ARIN data from meraki_inventory: WAN1={result['wan1_arin_provider']}, WAN2={result['wan2_arin_provider']}")
        else:
            # Only query ARIN if we don't have a meraki_device record
