#!/usr/bin/env python3
"""
Database-driven Bulk DDNS Enablement - Excluding Hub-tagged Devices
Uses device tags from database to properly exclude hub devices
"""
...
def get_networks_from_database():
    """Get all MX networks from database, excluding hub-tagged devices"""
    print(f'[{datetime.now().strftime("%H:%M:%S")}] Connecting to database...')
    
    conn = psycopg2.connect(Config.SQLALCHEMY_DATABASE_URI)
    cur = conn.cursor()
    
    # Get all MX networks but exclude those with hub tags
    query = '''
    SELECT DISTINCT network_id, network_name, device_serial, device_model, device_tags
    ...
    for network_id, network_name, device_serial, device_model, device_tags_json in results:
        # Parse device tags
        device_tags = []
        if device_tags_json:
            try:
        ...
        # Check if any tag contains 'hub' (case insensitive)
        is_hub = any('hub' in str(tag).lower() for tag in device_tags)
    ...
    print(f'[{datetime.now().strftime("%H:%M:%S")}] Found {len(networks)} store networks')
    print(f'[{datetime.now().strftime("%H:%M:%S")}] Excluded {len(hub_networks)} hub-tagged networks')
    
    if hub_networks:
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Hub networks excluded:')
        for hub in hub_networks[:5]:  # Show first 5
            print(f'  - {hub["name"]} (tags: {hub["tags"]})')
        if len(hub_networks) > 5:
            print(f'  ... and {len(hub_networks) - 5} more')
...
def main():
    ...
    print('=== Database-driven Bulk DDNS Enablement (No Hubs) ===')
    print(f'Started at: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
    
    # Get all non-hub networks from database
    ...
    # Process all networks (remove safety limit)
    batch_size = 10
    total_enabled = 0
    ...
    estimated_minutes = (len(networks) * 0.5) / 60  # 0.5 seconds per network
    
    print(f'[{datetime.now().strftime("%H:%M:%S")}] Will process {total_batches} batches')
    print(f'[{datetime.now().strftime("%H:%M:%S")}] Estimated time: {estimated_minutes:.1f} minutes')
    print()
    ...
    for batch_num in range(total_batches):
        ...
        # Shorter delay between batches for efficiency
        if start_index + batch_size < len(networks):
            print(f'[{datetime.now().strftime("%H:%M:%S")}] Waiting 3 seconds before next batch...')
            time.sleep(3)
