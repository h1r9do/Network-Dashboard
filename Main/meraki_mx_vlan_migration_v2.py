#!/usr/bin/env python3
"""
Meraki MX VLAN Migration Script v2
==================================
...
New in v2:
- Creates group policies before VLANs
- Preserves all DHCP/DNS settings
- Handles DHCP relay configurations
- Better error handling for dependencies

Usage:
    python3 meraki_mx_vlan_migration_v2.py --network-id <network_id> --source-config <json_file> --mode <test|production>
...
class VLANMigrator:
    def __init__(self, network_id, mode='production'):
        ...
        self.log(f"VLAN Migrator v2 initialized in {self.mode.upper()} mode")
    ...
    def migrate_vlans(self, source_vlans, source_policies=None):
        ...
        """
        self.log("Starting VLAN migration v2...")
        ...
        if last_vlan_id and last_vlan_id not in [100, 200, 300, 301, 400, 410, 800, 803, 900]:
            self.log(f"\nCleaning up temporary VLAN {last_vlan_id}...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Meraki MX VLAN Migration Tool v2')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        print(__doc__)
        print("\nExample usage:")
        print("  python3 meraki_mx_vlan_migration_v2.py --network-id L_12345 --source-config azp_30_config.json --mode test")
        print("  python3 meraki_mx_vlan_migration_v2.py --network-id L_12345 --source-config azp_30_config.json --mode production")
    else:
        main()