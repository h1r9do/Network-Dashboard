#!/usr/bin/env python3
"""
Populate provider_match_test table with provider matching logic
"""
...
def populate_test_data():
    """Populate the test table with real data"""
    
    print("Populating provider matching test data...")
    ...
    # Clear existing test data (except the sample data)
    cursor.execute("DELETE FROM provider_match_test WHERE site_id NOT LIKE 'TEST%'")
    
    # Get a sample of DSR circuits (limit to first 100 for testing)
    cursor.execute("""
        SELECT 
            c.site_id,
            c.site_name,
    ...
    print(f"Processing {len(circuits)} DSR circuits...")
    ...
    for circuit in circuits:
        ...
        try:
            ...
            print(f"Error querying Meraki data for {site_name}: {e}")
            continue
    ...
    cursor.execute("""
        SELECT site_name, circuit_purpose, dsr_provider, arin_provider,
               provider_match_confidence, match_reason
    ...
    for row in cursor.fetchall():
        print(f"{row['site_name']} ({row['circuit_purpose']}): {row['dsr_provider']} → {row['arin_provider']} ({row['provider_match_confidence']}%)")
...
if __name__ == "__main__":
    try:
        processed, matched = populate_test_data()
        print(f"\nTest data population complete! {matched}/{processed} circuits matched.")
    except Exception as e:
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()