...
def dsrcircuits():
    ...
    try:
        ...
        for circuit, meraki_device in enriched_circuits:
            ...
            # Check if site is in remodeling
            is_remodeling = False
            if circuit.device_tags:
                if isinstance(circuit.device_tags, str):
                    try:
                        tags = json.loads(circuit.device_tags)
                        is_remodeling = 'Remodeling' in tags
                    except:
                        is_remodeling = 'Remodeling' in circuit.device_tags
                elif isinstance(circuit.device_tags, list):
                    is_remodeling = 'Remodeling' in circuit.device_tags
            
            grouped_data.append({
                'network_name': circuit.network_name,
                'device_tags': circuit.device_tags or [],
                'is_remodeling': is_remodeling,
        ...
        for entry in grouped_data:
            ...
            # Count Starlink by satellite speed (more accurate)
            if entry['wan1'].get('speed') and entry['wan1']['speed'].lower() == 'satellite':
                starlink_count += 1
            ...
            # Count Starlink by satellite speed (more accurate)
            if entry['wan2'].get('speed') and entry['wan2']['speed'].lower() == 'satellite':
                starlink_count += 1
...
def confirm_circuit(site_name):
    ...
    # Add device_serial to response if not already present
    if 'device_serial' not in result and not result.get('error'):
        # Get device serial from MerakiInventory
        ...
        if meraki_device:
            result['device_serial'] = meraki_device.device_serial
...
def refresh_arin_data(site_name):
    """Refresh ARIN data for a site via real-time DDNS/Meraki API lookup"""
    try:
        import requests
        import os
        import socket
        ...
        # Get device serial for this site
        meraki_device = MerakiInventory.query.filter(
            func.lower(MerakiInventory.network_name) == func.lower(site_name)
        ).first()
        
        if not meraki_device or not meraki_device.device_serial:
            return jsonify({
                'success': False,
                'error': f'No Meraki device found for {site_name}'
            }), 404
        
        # Query real-time uplink statuses from Meraki API
            ...
            for device_status in all_uplinks:
                if device_status.get('serial') == device_serial:
                    ...
                    for uplink in uplinks:
                        ...
                        # Get public IP - prefer publicIp, then try DDNS resolution
                        public_ip = None
                        
                        # First try publicIp field
                        if uplink.get('publicIp'):
                            public_ip = uplink.get('publicIp')
                            logger.info(f"Got publicIp for {interface}: {public_ip}")
                        
                        # If no publicIp, try DDNS resolution
                        elif uplink.get('ddnsHostnames'):
                            ddns_hostnames = uplink.get('ddnsHostnames')
                            if ddns_hostnames and len(ddns_hostnames) > 0:
                                ddns_hostname = ddns_hostnames[0]
                                try:
                                    resolved_ip = socket.gethostbyname(ddns_hostname)
                                    # Verify it's a public IP
                                    ip_obj = ipaddress.ip_address(resolved_ip)
                                    if not (ip_obj.is_private or ip_obj.is_loopback or ip_obj.is_link_local):
                                        public_ip = resolved_ip
                                        logger.info(f"Resolved DDNS {ddns_hostname} to {public_ip}")
                                except Exception as e:
                                    logger.warning(f"Failed to resolve DDNS {ddns_hostname}: {e}")
                        
                        # Store the public IP
                        if public_ip:
                            if interface == 'wan1':
                                result['wan1_ip'] = public_ip
                            elif interface == 'wan2':
                                result['wan2_ip'] = public_ip
                    
                    result['source'] = 'meraki_api_realtime'
                    break
            
            if not device_found:
                return jsonify({
                    'success': False,
                    'error': f'Device {device_serial} not found in uplink statuses'
                }), 404
                
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to get Meraki uplink data: {e}")
            return jsonify({
                'success': False,
                'error': f'Failed to query Meraki API: {str(e)}'
            }), 500
        ...
        if not result['wan1_ip'] and not result['wan2_ip']:
            return jsonify({
                'success': False,
                'error': 'No public IP addresses found for this device',
                'warnings': ['Device may not have active uplinks or public IPs']
            }), 404
        
        # Initialize in-memory cache for this request
        rdap_cache = {}
        
        # Query ARIN RDAP for provider information
        def query_arin_rdap(ip):
            """Query ARIN RDAP with simple ISP-focused parsing"""
            if not ip or ip == '0.0.0.0':
                return 'Unknown'
            
            # Check cache first
            if ip in rdap_cache:
                logger.info(f"Using cached RDAP for {ip}: {rdap_cache[ip]}")
            ...
            try:
                ...
                # Special Verizon range
                if ip.startswith("166.80."):
                    provider = "Verizon Business"
                    rdap_cache[ip] = provider
                    return provider
            ...
            try:
                logger.info(f"Querying RDAP for {ip}")
                rdap_url = f"https://rdap.arin.net/registry/ip/{ip}"
                rdap_response = requests.get(rdap_url, timeout=10)
                
                if rdap_response.status_code == 200:
                    rdap_data = rdap_response.json()
                    
                    # Collect all organization entities with their roles
                    entities = rdap_data.get('entities', [])
                    org_names = []
                    
                    # Recursively collect entities
                    def collect_entities(entity_list):
                            ...
                            if len(vcard_array) > 1 and isinstance(vcard_array[1], list):
                                ...
                                for vcard_item in vcard_array[1]:
                                    if isinstance(vcard_item, list) and len(vcard_item) >= 4:
                                        if vcard_item[0] == 'fn':  # Full Name
                                            name = str(vcard_item[3]).strip()
                                        elif vcard_item[0] == 'kind':
                                            kind = vcard_item[3]
                                
                                # Only collect organization entities (not individuals)
                                if name and kind == 'org':
                                    roles = entity.get('roles', [])
                                    # Prioritize registrant over ISP
                                    priority = 1 if 'registrant' in roles else 2 if 'ISP' in roles else 3
                                    org_names.append((name, priority))
                            
                            # Check nested entities
                            if 'entities' in entity:
                                collect_entities(entity.get('entities', []))
                    
                    collect_entities(entities)
                    
                    # Sort by priority (registrant first, then ISP, then others)
                    org_names.sort(key=lambda x: x[1])
                    
                    if org_names:
                        # Use the highest priority organization
                        provider = org_names[0][0]
                        
                        # Apply known normalizations
                        company_map = {
                            "Comcast Cable Communications, LLC": ["Comcast Cable Communications", "CCCS"],
                            "Verizon Business": ["MCICS", "MCI Communications Services"],
                            "AT&T": ["AT&T Internet", "AT&T Services", "ATT-INTERNET4"],
                            "Charter Communications": ["Charter Communications", "CHARTER-COMMUNICATIONS"],
                            "CenturyLink": ["CenturyLink", "CENTURYLINK-US-LEGACY"],
                            "Frontier Communications": ["Frontier Communications", "FRONTIER-FRTR"],
                            "Cox Communications": ["Cox Communications", "COX-COMMUNICATIONS"],
                        }
                        
                        for canonical, variations in company_map.items():
                            for variant in variations:
                                if variant.lower() in provider.lower() or provider.lower() in variant.lower():
                                    provider = canonical
                                    break
                        
                        logger.info(f"RDAP found provider for {ip}: {provider}")
                        rdap_cache[ip] = provider
                        return provider
                    
                    # Fallback to network name if no entities found
                    if 'name' in rdap_data:
                        provider = rdap_data['name']
                        rdap_cache[ip] = provider
                        return provider
                        
                return 'Unknown'
            except Exception as e:
                logger.error(f"RDAP lookup failed for {ip}: {e}")
                return 'Unknown'
        
        # Look up providers for the IPs
        if result['wan1_ip']:
        ...
        return jsonify({
            'success': True,
            'wan1_ip': result['wan1_ip'],
            'wan1_arin_provider': result['wan1_arin_provider'],
            'wan2_ip': result['wan2_ip'],
        ...
        logger.error(f"Unexpected error in refresh_arin_data: {e}")
        return jsonify({
            'success': False,
...
def update_device_tags(device_serial):
    ...
    try:
        ...
        if check_response.status_code == 404:
            # Device not found in Meraki, skip API update but update database
            meraki_device.device_tags = new_tags
            db.session.commit()
            
            return jsonify({
                'success': True,
                'message': 'Tags updated in database only (device not found in Meraki)',
                'updated_tags': new_tags,
                'device_serial': device_serial,
                'meraki_updated': False
            })
        elif check_response.status_code != 200:
            return jsonify({'success': False, 'error': f'Error checking device in Meraki: {check_response.status_code}'}), 500
        
        # Update tags in Meraki
        update_url = f"https://api.meraki.com/api/v1/devices/{device_serial}"
        ...
        if update_response.status_code == 200:
            ...
            return jsonify({
                'success': False, 
                'error': f'Failed to update tags in Meraki: {update_response.status_code} - {update_response.text}'
            }), 500
...
@dsrcircuits_bp.route('/api/get-device-tags/<device_serial>', methods=['GET'])
def get_device_tags(device_serial):
    """Get current tags for a device"""
    try:
        meraki_device = MerakiInventory.query.filter_by(device_serial=device_serial).first()
        if not meraki_device:
            return jsonify({'success': False, 'error': 'Device not found'}), 404
            
        tags = meraki_device.device_tags or []
        
        return jsonify({
            'success': True,
            'tags': tags,
            'device_serial': device_serial,
            'network_name': meraki_device.network_name
        })
        
    except Exception as e:
        logger.error(f"Error getting device tags for {device_serial}: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
