...
def daily_enablement_data():
    """Get daily enablement data from database ONLY"""
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT summary_date, daily_count FROM enablement_summary ORDER BY summary_date ASC")
        ...
        # Calculate summary from daily_data
        if daily_data:
            total_enabled = sum(item['count'] for item in daily_data)
            avg_per_day = round(total_enabled / len(daily_data), 1)
            max_day = max(daily_data, key=lambda x: x['count'])
            
            summary = {
                'total_enabled': total_enabled,
                'avg_per_day': avg_per_day,
                'max_day': {'count': max_day['count'], 'date': max_day['date']},
                'days_analyzed': len(daily_data),
            ...
            summary = {
                'total_enabled': 0, 'avg_per_day': 0, 'max_day': {'count': 0, 'date': ''},
                'days_analyzed': 0, 'period_start': '', 'period_end': '', 'last_updated': datetime.now().isoformat()
            }
        ...
        logging.info(f"📊 DATABASE: {len(daily_data)} days, {total_enabled} enablements")
        return jsonify(response)
        
    except Exception as e:
        logging.error(f"Database error in daily_enablement_data: {str(e)}")
        return jsonify({'error': f'Database error: {str(e)}', 'data_source': 'database_error'}), 500
    ...
    try:
        ...
        for row in results:
            enablements.append({
                'date': str(row[0]),
                'site_name': row[1],
                'circuit_purpose': row[2] or 'Unknown',
                'provider': row[3] or 'Unknown',
                'previous_status': row[4] or 'Unknown',
                'current_status': row[5],
                'assigned_to': row[6] or 'Unassigned',
                'sctask': row[7] or '',
                'enabled_at': row[8].isoformat() if row[8] else str(row[0])
            })
...
def closure_attribution_data():
    ...
    try:
        ...
        # Get all dates in the range from enablement_summary
        if date_param:
            if isinstance(date_param, tuple):
                cursor.execute(f"""
                    SELECT summary_date, 0 as placeholder
                    FROM enablement_summary
                    {date_filter}
        ...
        if days is not None:
            cursor.execute("""
                SELECT 
                    ...
                    COALESCE(de.sctask, '') as sctask_number
